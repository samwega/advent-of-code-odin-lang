...
differences: sa.Small_Array(16, int)
checksum: int

main :: proc() {

	for row in 0..<16 {
		smallest := input[row][0]
		largest := input[row][0]
		for col in 0..<16 {
			val := input[row][col]
			if val < smallest {
...
// I used `search & replace` preprocessing to get the proper format for the values and make things easier. People are solving Advent of Code inside Minecraft or with pen & paper so I don't think it's cheating per se.
input := [16][16]int{
